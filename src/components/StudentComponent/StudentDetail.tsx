    ...
    <div className="p-4 max-w-5xl mx-auto">
      ...
      <div className="bg-green-100 shadow rounded-lg p-6 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm text-gray-700">
          <div><span className="font-semibold">Name:</span> {studentData.name}</div>
          <div><span className="font-semibold">Email:</span> {studentData.email}</div>
          <div><span className="font-semibold">Roll No:</span> {studentData.rollNo}</div>
          <div><span className="font-semibold">Phone:</span> {studentData.phone}</div>
          <div><span className="font-semibold">Total Uploaded Points:</span> {totalUploaded}</div>
          <div><span className="font-semibold">Total Approved Points:</span> {totalApproved}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {Object.entries(studentData.points).map(([year, data]) => (
          <div key={year} className="bg-red-100 rounded-lg shadow p-4">
