...
const StudentYearlyDetails = ({ data }) => {
    ...
    const handleSubmit = (e: any) => {
        ...
        if ((Number(formData.techFestOrganizer) + data[1].subpoints[0].already_acquired) > 10) {
            toast.error("2.a Tech Fest/Fest/Teachers Day/Fresher’s Welcome Organizer can't be greater than 10");
            return;
        }
        if ((Number(formData.techFestParticipant) + data[1].subpoints[1].already_acquired) > 6) {
            toast.error("2.b Tech Fest/Fest/Teachers Day/Fresher’s Welcome Participant can't be greater than 6");
            return;
        }
        ...
        if ((Number(formData.treePlantation) + data[3].already_acquired) > 10) {
            toast.error("4. Tree plantation and Up-keeping (per tree) can't be greater than 10");
            return;
        }
        if ((Number(formData.reliefFundCollection) + Number(formData.reliefWorkTeam) + data[4].already_acquired ) > 40) {
        ...
        if ((Number(formData.participationInArts) + data[5].already_acquired ) > 20) {
            toast.error("6. Participation in Debate/Group Discussion/Workshop/Tech quiz/Music/Dance/Drama/Elocution/Quiz/Seminar/Painting/any Performing Arts/Photography/Film Making can't be greater than 40");
            return;
        }
        if ((Number(formData.publication) + data[6].already_acquired ) > 20) {
            toast.error("7. Publication in News Paper, Magazine, Wall Magazine & Blogs can't be greater than 20");
            return;
        }
        if ((Number(formData.researchPublication) + data[7].already_acquired ) > 30) {
            toast.error("8. Research Publication (per publication) can't be greater than 30");
            return;
        }
         if ((Number(formData.innovativeProjects) + data[8].already_acquired ) > 60) {
            toast.error("9. Innovative Projects (other than course curriculum) can't be greater than 60");
            return;
        }
        if ((Number(formData.bloodDonation) + data[9].subpoints[0].already_acquired ) > 16) {
            toast.error("10.a Blood donation can't be greater than 16");
            return;
        }
        if ((Number(formData.bloodDonationCampOrganization) + data[9].subpoints[1].already_acquired ) > 20) {
            toast.error("10.b Blood Donation Camp Organization can't be greater than 20");
            return;
        }
        if ((Number(formData.sportsPersonal) + data[10].subpoints[0].already_acquired ) > 20) {
            toast.error("11.a Sports/Games/Adventure Sports/Trekking/Yoga (Personal Level) Camp can't be greater than 20");
            return;
        }
        if ((Number(formData.sportsCollege) + data[10].subpoints[1].already_acquired ) > 10) {
            toast.error("11.b Sports/Games/Adventure Sports/Trekking/Yoga (College Level) Camp can't be greater than 10");
            return;
        }
         if ((Number(formData.sportsUniversity) + data[10].subpoints[2].already_acquired ) > 20) {
            toast.error("11.c Sports/Games/Adventure Sports/Trekking/Yoga (University Level) Camp can't be greater than 20");
            return;
        }
         if ((Number(formData.sportsDistrict) + data[10].subpoints[3].already_acquired ) > 24) {
            toast.error("11.d Sports/Games/Adventure Sports/Trekking/Yoga (District Level) Camp can't be greater than 24");
            return;
        }
         if ((Number(formData.sportsState) + data[10].subpoints[4].already_acquired ) > 30) {
            toast.error("11.e Sports/Games/Adventure Sports/Trekking/Yoga (State Level) Camp can't be greater than 30");
            return;
        }
         if ((Number(formData.sportsNationalInternational) + data[10].subpoints[5].already_acquired ) > 40) {
            toast.error("11.f Sports/Games/Adventure Sports/Trekking/Yoga (National/International Level) Camp can't be greater than 40");
            return;
        }
    ...
    return (
        <div>
            ...
            >
                Submit
            </button> Total Points: {totalPoint}
